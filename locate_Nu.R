...
beep_data <- beep_data %>% 
  filter (!is.na(NodeId))
#beep_data$NodeId=gsub("3.28E+60","328E58",beep_data$NodeId)
str(beep_data)
#beep_data <- read.csv("/Users/gamageperera/Desktop/Motus/Motus/data_tools_master/beep_data.csv", as.is=TRUE, na.strings=c("NA", "")) 
#str(beep_data)
#beep_data <- beep_data[beep_data$Time > as.POSIXct("2021-01-19"),]
...
#tags <- read.csv("/Users/gamageperera/Desktop/Motus/Motus/data_tools_master/Winterbirds_stationbeeps/Winterbirds stationbeep_ 2021-09-16_220737 .csv", as.is=TRUE, na.strings=c("NA", "")) #uppercase node letters
#filter(Validated ==1)
...
actbud<- read.csv("/Users/gamageperera/Desktop/Motus/Motus/data_tools_master/actbud.csv")
...
start.date = "20210120"; end.date = "20210328"
Dates <- seq(ymd(start.date),ymd(end.date), by = "days")
# install.packages("tidyverse")
library(tidyverse)
sun_df <- expand.grid(Dates = Dates,tag_id = actbud$TagId) %>% 
  left_join(actbud) %>%
  group_by(Dates, TagId, Latitude, Longitude) %>% 
