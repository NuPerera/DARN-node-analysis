...
file_types <- c("gps","telemetry")  #"data", "node-data", "gps") #, "log", "telemetry", "sensorgnome") #Select the filetypes you need
begin <- "2021-01-19 23:00:00 UTC"
enddate <- "2021-03-28 16:00:00 UTC"
...
post <- function(endpoint, payload=NULL) {
  ...
  response <- POST(host, path = endpoint, body=payload_to_send,encode="json", timeout(360)) 
...
getStationFileList <- function(station_id, begin, filetypes=file_types, end=enddate) {
  endpoint <- files
  payload <- list("station-id" = station_id, begin = as.Date(begin)) #"station_id" can be manually set to one sensorstation, if needed
  if (!is.null(filetypes)) {
    ...
    payload[['file-types']] = filetypes  #add_types
  } 
...
get_data <- function(project, outpath, f=NULL) {
  ...
  files_avail <- lapply(my_stations[["stations"]], function(station) {
    ...
    kwargs <- list(
      ...
      if(is.null(begin)){
      begin = as.POSIXct(station[['deploy-at']],format="%Y-%m-%dT%H:%M:%OS",tz = "UTC", optional=TRUE)} else {begin = begin}
    )
  ...
  return(outfiles)})
  print(object.size(files_avail), units = "Mb")
  
  filenames <- unname(rapply(files_avail, grep, pattern = "CTT", value=TRUE)) 
  print(c("filenames unlisted at", Sys.time()))
  files_to <- filenames[!filenames %in% files_loc]
  remain.rows <- as.integer(length(files_to))
  print(paste(remain.rows,"files to be downloaded"))
  files_to_download <<- as.data.frame(files_to) #It would be nice to update straight off of this, rather than having to look all this up again on each attempt

  allfiles <- rapply(files_avail, function(z) z %in% files_to[1:remain.rows], how = "unlist") #You can specify the rows of "files_to" you want
  print(c("files available listed at", Sys.time()))
  ...
  get_files <- function(x, y) {
    ...
    if (filetype != "log" & filetype != "telemetry" & filetype != "sensorgnome") {
      ...
      print(x)
      #contents <- na.omit(contents) #to get rid of rows in node.health files with "NA"s that are incompatible with Postgres. Is there another way?
...
get_my_data <- function(my_token, outpath, db_name){ #=NULL) { #Shouldn't this just be db_name = db_name?
  projects <- content(POST(host, path = project, body = list(token=my_token), encode="json", verbose()), as = "parsed")
